...
const connection: ModelConnection = { api: "openai-completions", baseUrl: "https://provider.example/v1", network: "remote", modelId: "vendor/model:latest", contextWindow: 64000, maxOutputTokens: 4096,
...
for (const backend of ["opencode", "pi"] as const) {
  for (const api of ["openai-completions", "openai-responses", "anthropic-messages"] as const) {
    test(`${backend} bootstrap writes native ${api} config before launch with literal credential handling`, () => {
      ...
      const env = buildContainerEnvironment({ PATH: "/bin", GH_TOKEN: "forge-secret", INPUT_MODEL_CREDENTIALS: "unused-secret" }, input, backend, versions);
        ...
        require: (name: string) => {
          if (name === "node:fs") return {
            ...
            writeFileSync: (path: string, value: string, options: { mode: number }) => {
              assert.equal(options.mode, 0o600); files.set(path, value);
            },
          };
      ...
      if (backend === "opencode") {
        ...
        assert.equal(parsed.provider["review-provider"].npm, api === "anthropic-messages" ? "@ai-sdk/anthropic" : api === "openai-responses" ? "@ai-sdk/openai" : "@ai-sdk/openai-compatible");
...
test("keyless configuration contains no real credential; network policy cannot enable host networking", () => {
  const local: ModelConnection = { ...connection, credential: undefined, network: "private", baseUrl: "http://host.docker.internal:11434/v1" };
  ...
  assert.equal(env.REVIEW_MODEL_TOKEN, undefined);
  const args = buildContainerArguments({ backend: "pi", connection: local, containerName: "test", containerEngine: "docker" });
...
for (const backend of ["opencode", "pi"] as const) {
  test(`${backend} keyless Responses bootstrap uses only a fixed non-secret placeholder`, () => {
    ...
    let launched = false;
    runInNewContext(SANDBOX_BOOTSTRAP, {
      process: { env }, console: { error: () => assert.fail("bootstrap failed") },
      require: (name: string) => name === "node:fs" ? {
        mkdirSync: () => undefined,
        writeFileSync: (path: string, content: string) => files.set(path, content),
      } : {
        spawn: () => { launched = true; return new EventEmitter(); },
      },
    });
    assert.ok(launched);
