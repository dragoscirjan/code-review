...
function github(): GitHubClient {
  return new GitHubClient("token", "https://api.example.test", async () =>
    new Response("not-a-real-tarball", { status: 200 }),
  );
}

function fakeRunner(
  calls: Array<{ command: string; args: string[]; environment: NodeJS.ProcessEnv }>,
): CommandRunner {
  return async (command, args, options) => {
    ...
    if (args.includes("index") || args.includes("analyze")) {
      const storage =
        options.environment.GITNEXUS_STORAGE_PATH ??
    ...
    adapter: {
      ...
      save: async (paths, key) => {
        ...
        const metadata = JSON.parse(
          await readFile(join(path, "code-review-cache.json"), "utf8"),
        ) as { createdAt: string };
  ...
  test(`installs, indexes, and queries with ${indexer}`, async () => {
    ...
    try {
      ...
      if (indexer === "cgc") {
        assert.ok(
          calls.some((call) =>
            call.args.includes("codegraphcontext==0.6.13"),
          ),
        );
        assert.ok(calls.some((call) => call.args.includes("--force")));
      } else {
        assert.ok(
          calls.some((call) => call.args.includes("gitnexus@1.6.12")),
        );
...
test(
  "terminates indexer descendant processes after a timeout",
...
test(
  "terminates indexer descendants after a non-zero exit",
  { skip: process.platform === "win32" },
  async () => {
...
test("builds a bounded search query and index context", () => {
  ...
  const limited = limitIndexContext(
    `\u001b[31m${"😀".repeat(20_000)}`,
  );
