...
const credentials = JSON.stringify({ router: { type: "bearer", value: "router-secret" }, unused: { type: "bearer", value: "unused-secret" } });
...
test("selects only the referenced provider credential without a model allowlist", () => {
  const connection = load({ ...config, model: { id: "vendor/paid-model:latest", contextWindow: 64000, maxOutputTokens: 4096 } });
...
test("supports keyless local endpoints and explicit authentication schemes", () => {
  const local = load({ ...config, provider: { api: "openai-completions", baseUrl: "http://192.168.1.8:11434/v1", network: "private" } }, "{}");
  ...
  for (const api of ["openai-completions", "openai-responses", "anthropic-messages"]) {
    ...
    assert.equal(load({ ...config, provider: { ...config.provider, api } }, JSON.stringify({ router: { type, value: "token" } })).api, api);
  }
  assert.throws(() => load({ ...config, provider: { ...config.provider, api: "anthropic-messages" } }), /requires api-key/);
  assert.throws(() => load(config, JSON.stringify({ router: { type: "api-key", value: "token" } })), /OpenAI APIs accept bearer/);
  assert.throws(() => load({ ...config, provider: { ...config.provider, api: "anthropic-messages" } },
    JSON.stringify({ router: { type: "api-key", value: "sk-ant-oat-sentinel" } })), /OAuth tokens are unsupported/);
});

test("rejects unknown/native harness fields, missing credentials and malformed inputs", () => {
  for (const invalid of [
    { ...config, version: 2 }, { ...config, plugin: ["evil"] },
  ...
  assert.throws(() => load(config, '{"router":{"type":"bearer","value":"sensitive'), (error: unknown) => {
...
test("requires explicit network consent and forbids endpoint interpolation/secrets", () => {
  for (const baseUrl of ["http://api.example.com/v1", "https://user:token@api.example.com/v1", "https://api.example.com/?key=secret",
    "https://api.example.com/v1#", "https://api.example.com/v1?",
    "https://api.example.com/#secret", "file:///tmp/key", "https://api.example.com/{file:/tmp/key}",
    "https://api.example.com/$SECRET", "https://localhost/v1", "https://127.0.0.1/v1", "https://[::1]/v1"]) {
    assert.throws(() => load({ ...config, provider: { ...config.provider, baseUrl } }));
  }
  assert.throws(() => load({ ...config, provider: { ...config.provider, network: undefined } }), /explicitly authorize/);
});

test("fails closed for private, reserved, metadata, mixed DNS and resolution errors", async () => {
  for (const ip of ["127.0.0.1", "169.254.169.254", "0.0.0.0", "100.100.100.200", "192.168.1.2", "10.1.2.3", "172.16.1.2", "::1", "::ffff:127.0.0.1", "fd00::1", "fe80::1", "2001:db8::1", "224.1.1.1"]) {
  ...
  await assert.rejects(validateModelEndpoint(load(), async () => [{ address: "8.8.8.8" }, { address: "10.1.1.1" }]), /network policy/);
  await assert.rejects(validateModelEndpoint(load(), async () => []), /network policy/);
  await assert.rejects(validateModelEndpoint(load(), async () => { throw new Error("sensitive"); }), /network policy/);
});
