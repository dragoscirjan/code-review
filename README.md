...
| `timeout-seconds`         | `600`                  | One call in single-pass mode; aggregate shard/merge-pass time in specialist mode.                 |
...
Inline findings are published in one bounded, deduplicated review batch. A model-proposed exact replacement becomes a GitHub suggested change only after the host maps its path, side, line, and original evidence to one exact `RIGHT`-side diff line. LEFT-side findings, prose fixes, no-op replacements, analyzer findings, unsafe replacement text, and findings that fail mapping remain prose-only or are rejected by normal finding validation. The action never applies or pushes the change; an authorized GitHub user may use **Commit suggestion**.
...
`review-strategy: auto` keeps the existing single review call for small, low-risk changes. It deterministically selects the sharded review when the authoritative full diff is truncated, has more than two commentable files, has more than 80 changed lines, exceeds 24 KiB, has partial analyzer coverage, or touches a fixed sensitive surface such as workflows/actions, authentication, credentials/secrets, configuration, manifests/lockfiles, migrations/schemas, API/routes/interfaces, or canonical container/build metadata (`Dockerfile`, `Containerfile`, bounded suffixed variants, Compose files, and action metadata). Sensitive names use exact normalized path segments, filenames, or dot/dash/underscore-delimited filename tokens rather than substring matches. `single-pass` and `specialists` (the retired role-pass name of the sharded route) force either path. Pull-request content can trigger only these fixed effort-allocation signals; it cannot add, remove, reorder, or retry shards.

Sharded review splits the selected diff into bounded whole-file shards (small files cluster up to four per shard; oversized files split at hunk boundaries; a single hunk larger than the shard cap is never cut and rides the bounded leftover shard with truncation-dropped files). Shards run sequentially; each receives one bounded diff section, bounded PR metadata, base guidance/configuration and issue criteria, the full code-index context union, and all prior findings, and reviews all four dimensions in one pass with at most three findings restricted to its authoritative paths. Every candidate passes the production strict result parser, exact changed-line/evidence mapper, confidence and secret policy before the merge pass. The host first collapses competing claims at the same path/side/line by severity, confidence, and deterministic tie-breakers, reserves one candidate for every review dimension, and fills the remaining ten-candidate budget by that global priority. The merge pass receives only those host-ID-labelled candidates, their exact hunks, at most 12 KB of base guidance/configuration/criteria, the immutable security policy, and one reject-only version 1 schema; it does not receive the finding-result contract. It can return only a duplicate-free subset of IDs to reject and cannot create, rewrite, rank, or suppress deterministic analyzer findings. Retained findings are validated again and merged with analyzers through the existing deduplicator and global ten-finding cap.

The action reserves one conservative token unit per UTF-8 prompt byte, 1,024 envelope units per call, every configured per-call maximum output, and the worst-case merge-pass prompt/output before the first shard. Each shard is capped at `min(model.maxOutputTokens, 4096)` and the merge pass at `min(model.maxOutputTokens, 2048)`. `specialist-token-budget` must be 20,000–2,000,000. This reservation is a host-side safety bound, not provider billing telemetry. `timeout-seconds` is one monotonic deadline: in sharded mode every shard and the merge pass share it, and each backend call propagates its remaining budget through endpoint resolution, workspace setup, process execution, termination grace, forced container removal, and workspace cleanup. A backend process is not started unless cleanup time remains reserved. Snapshot, linked-issue, and managed-summary lease freshness are checked around every phase. A timeout, stale input, malformed/secret-bearing output, path breach, budget breach, cleanup/backend failure, or merge-pass violation aborts before any write; there is no partial publication, retry, or fallback.

All shards and the merge pass use the same configured model/provider and selected provider credential in separate mount-free sandbox calls. They are not an independent security quorum, and a provider may correlate the requests. Sharded mode increases provider calls, cost, credential-bearing harness starts, and the existing DNS/redirect/supply-chain exposure. GitHub credentials and unused provider credentials remain host-side.
