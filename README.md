...
- **Redirect policy.** Cross-origin redirects are blocked at the gateway; same-origin redirects pass through and re-enter the gateway, so every credential-carrying connection stays under host-side destination policy.
...
- **Proxy bypass.** The gateway connects with the pinned address directly; hostile `HTTP(S)_PROXY` environment variables cannot reroute upstream connections, and the container environment never receives proxy variables.
