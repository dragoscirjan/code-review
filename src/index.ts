...
async function main(): Promise<void> {
  ...
  const policyDigest = reviewPolicyDigest({
    ...
    specialistPolicy: {
      ...
      maximumRolePasses: SHARD_REVIEW_DIMENSIONS.length,
      maximumArbiterPasses: 1,
      maximumFindingsPerRole: MAX_FINDINGS_PER_SHARD,
      maximumRawFindings: MAX_RAW_SHARD_FINDINGS,
      maximumArbiterCandidates: MAX_ARBITER_CANDIDATES,
      maximumSpecialistContextBytes: MAX_SHARD_CONTEXT_BYTES,
      maximumArbiterContextBytes: MAX_ARBITER_CONTEXT_BYTES,
      maximumArbiterPromptBytes: MAX_ARBITER_PROMPT_BYTES,
      specialistRequestOverheadTokens: SHARD_REQUEST_OVERHEAD_TOKENS,
      specialistOutputTokens: shardOutputTokens(config.connection.maxOutputTokens, config.connection.reasoning),
